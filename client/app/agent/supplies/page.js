...
export default function GetAllProducts() {
  ...
  const getVerificationBadge = (status) => {
    switch (status) {
      case "verified":
        return (
          <span className="flex items-center bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 rounded-full px-3 py-1 text-xs font-medium">
            <BadgeCheck className="w-4 h-4 mr-1" /> Verified
          </span>
        );
      case "pending":
        return (
          <span className="flex items-center bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 rounded-full px-3 py-1 text-xs font-medium">
            <Clock className="w-4 h-4 mr-1" /> Pending
          </span>
        );
      case "rejected":
        return (
          <span className="flex items-center bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 rounded-full px-3 py-1 text-xs font-medium">
            <XCircle className="w-4 h-4 mr-1" /> Rejected
          </span>
        );
      default:
        return (
          <span className="flex items-center bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 rounded-full px-3 py-1 text-xs font-medium">
            Unverified
          </span>
        );
    }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
              Browse through products from all suppliers. Each item comes with
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-start">
                          ...
                          <div>
                            ...
                            <p className="text-sm">
                              {item.supplier.deliveryRadius.radiusInKm} km
                              around location
                            </p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {/* ({item.supplier.coordinates.lat.toFixed(4)},{" "}
                              {item.supplier.coordinates.lng.toFixed(4)}) */}
                            </p>
                          </div>
                        </div>
                      </div>
          ...
          {!isLoading && !error && inventoryItems.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 dark:text-gray-300 max-w-md mx-auto">
                Currently there are no inventory items available. Please check
                back later.
              </p>
            </div>
          )}
